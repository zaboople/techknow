...
function pickMsgToSend(user, list, key, replyTo, forceReply) {
    ...
    if (replyTo!=null) {
        let keyIndex = 0;
        const makeReplyTo = (extra) =>
            makeReplyToSpan(keyIndex++, replyTo+extra);
...
UserMsgInput.propTypes = {
    // Or else linter screams:
    userName: PropTypes.string,
    remoteUsers: PropTypes.array.isRequired,
...
function Discussion({userName}) {
    ...
    const realMsgs = messages.length == 0
        ?[makeMsg("Loading123", null, [<i key={"xxxx"}>Loading<DotDot/></i>])]
        :messages;
    return <>
