...
class TaskManager(val runner:Runner) extends Actor {
  ...
  private def adjustWaiterCountIf {
    ...
    if (adjTasksComplete>=waiterLength && taskListReversed.size> waiterLength) 
      adjustWaiterCount
  }
  ...
  private def addWaiter{
    debug("*** WAITER ADDED ***")
    waiterList=new WaiterContainer(new Waiter(this, waiterLength, runner))::waiterList
    adjustWaiters
  }
